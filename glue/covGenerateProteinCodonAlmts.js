...
_.each(featuresList, function(featureObj) {
	...
	glue.inMode("module/covFastaProteinAlignmentExporterSeqIdOnly", function() {
		glue.command(["export", "AL_GISAID_CONSTRAINED", "-s", selectorModuleName, "-w", "sequence.analyse_aa_replacements = true", "-o", "alignments/cov_"+featureObj.name+".faa"]);
	});
	glue.inMode("module/covFastaAlignmentExporterSeqIdOnly", function() {
		glue.command(["export", "AL_GISAID_CONSTRAINED", "-s", selectorModuleName, "-w", "sequence.analyse_aa_replacements = true", "-o", "alignments/cov_"+featureObj.name+".fna"]);
	});
});