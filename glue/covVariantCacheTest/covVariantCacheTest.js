 ...
 * 1. If variants are not in the cache, sequences are not in the cache, then variants and associations are created.
	...
	c. Delete variant and association from DB
 ...
 * 3. If both are in the cache, variants and associations are created during populate-from-cache
	a. Restore DB and cache files
	b. Delete variant and association from DB
	d. Run populate scripts with appropriate sequence tweak
	e. Check that variation and associations created.
 */

// this is the check step
glue.inMode("sequence/cov-gisaid/EPI_ISL_500981", function() {
	glue.logInfo("EPI_ISL_500981 replacements", 
			glue.getTableColumn(glue.command(["list", "custom-table-row", "cov_replacement_sequence", "id"]), "id"))
	glue.logInfo("EPI_ISL_500981 NT insertions", 
			glue.tableToObjects(glue.command(["list", "custom-table-row", "cov_nt_insertion_sequence", "id"]), "id"))
	glue.logInfo("EPI_ISL_500981 insertions", 
			glue.tableToObjects(glue.command(["list", "custom-table-row", "cov_insertion_sequence", "id"]), "id"))
});
...
glue.inMode("sequence/cov-gisaid/EPI_ISL_465549", function() {
	...
	glue.logInfo("EPI_ISL_465549 NT deletions", 
			glue.tableToObjects(glue.command(["list", "custom-table-row", "cov_nt_deletion_sequence", "id"]), "id"))
	glue.logInfo("EPI_ISL_465549 deletions", 
			glue.tableToObjects(glue.command(["list", "custom-table-row", "cov_deletion_sequence", "id"]), "id"))
});
